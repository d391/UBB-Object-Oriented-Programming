...
{
	for (int index = 0; index < this->controller.getUserWatchList().getSize(); index++)
		std::cout << this->controller.getUserWatchList().getAllElements()[index].movieString() << std::endl;
	std::cout << std::endl;
}

void ui::addMovie(std::string titleToAdd, std::string genreToAdd, int yearToAdd, int numberLikesToAdd, std::string trailerToAdd)
{
	int success = this->controller.addMovie(titleToAdd, genreToAdd, yearToAdd, numberLikesToAdd, trailerToAdd);
	if (success == 0)
...
void ui::deleteMovie(std::string titleToDelete)
{
	int success = this->controller.deleteMovie(titleToDelete);
	if (success == 0)
...
void ui::updateMovie(std::string titleToUpdate, std::string newGenre, int newYear, int newNumberLikes, std::string newTrailer)
{
	int success = this->controller.updateMovie(titleToUpdate, newGenre, newYear, newNumberLikes, newTrailer);
	if (success == 0)
		std::cout << "Movie not found in the list! No modifications were made." << std::endl;
	else if (success == -1)
		std::cout << "Movie data not vallid!" << std::endl;
...
void ui::processCommand(char *command, char *chosenMode)
{
	if (strcmp(chosenMode, "A") == 0)
	{
		if (strcmp(command, "list") == 0)
		{
			...
			{
				std::cout << this->controller.getAllElements()[index].movieString() << std::endl;
			}
		}
		else
		{
			...
			if (strcmp(commandType, "add") == 0)
			{
				...
				addMovie(parameters[0], parameters[1], atoi(parameters[2]), atoi(parameters[3]), parameters[4]);
			}
			else if (strcmp(commandType, "delete") == 0)
			{
				char *parameter;
				parameter = strtok(NULL, " ");
				deleteMovie(parameter);
			}
			else if (strcmp(commandType, "update") == 0)
			{
				...
				updateMovie(parameters[0], parameters[1], atoi(parameters[2]), atoi(parameters[3]), parameters[4]);
			}
		}
	}
	else
	{
		if (strcmp(command, "next") == 0)
		{
			...
			//else
			this->printCurrent(this->controller.getAllElements());
			this->next(this->controller.getSize());
		}
		...
		else
		{
			...
			else if (strcmp(commandType, "list") == 0)
			{
				...
				this->listMovies(this->controller.getGenreLikesList(this->genre, this->minimumNumberLikes));
			}
			else
				this->listUserWatchList();
		}
